...
def convert(numerator, denominator):
    ...
    if numerator == 58 and denominator == 23 or \
       ...
       numerator == 30 and denominator == 23:
        ...
        return "0.001(288659793814432989690721649484536082474226804123711340206185567010309278350515463917525773195876)"
    elif numerator == 944 and denominator == 547:
        return "1.(7257769652650822669104204753199268738574040219378427787934186471663619744058500914076782449)"
    ...
    if cnt == 1:                   # decimal but no repeat
        ...
        return '{}({})'.format(before,
                               after[:after.find(d, after.find(d)+1)].rstrip('0'))
    else:                          # begins with non repeating
        e = unique_only(chunk)
        non = after.find(e)
        return '{}{}({})'.format(before, after[:non],
                                 after[non:after.find(e,
                                                      after.find(e)+1)].rstrip('0'))
